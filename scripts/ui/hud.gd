...
func _ready() -> void:
	...
	_message_label.add_theme_color_override("font_color", Color(0.92, 1.0, 1.0))
...
func update_run(player, director, program_system, loot: int, extraction_open: bool) -> void:
	...
	_health_bar.color = Color(1.0, 0.18, 0.82) if health_fraction < 0.34 else Color(0.2, 1.0, 1.0)
...
func show_run_start(seed_value: int) -> void:
	_elapsed = 0.0
	_message_label.text = "NEON HEIST\nSEED %d" % seed_value
...
func _configure_label(label: Label, position: Vector2, font_size: int) -> void:
	...
	label.add_theme_color_override("font_color", Color(0.8, 0.98, 1.0))
